...
int condition(LONG first, LONG end, LONG min , LONG Optimal)
{
	int i, j, o = 0, n = 0;
...
int main()
{
	LONG N, S, T, i, j, z, k, mm, min, con_num, chance, count = 0, temp , Optimal = 0,jj;
	long  long int  M, L = 0;
	bool found, rep;
	SOL ss;
	string* SS;
	...
	for (i = 0; i < E.size(); i++)
	{
		for (jj = 0; jj < repitedly.size(); jj++)
		{
			if (E[i].BEG == repitedly[jj].BEG && E[i].END == repitedly[jj].END && E[i].LEN == repitedly[jj].LEN)
			{
				///////////////////////////////////////////////////////////////////////////
				temp = matrix[E[i].BEG - 1][E[i].END - 1];
				matrix[E[i].BEG - 1][E[i].END - 1] = repitedly[jj].LEN;
				rep = true;
				for (k = 0; k < ANSWER.size(); k++)
				{
					for (z = 0; z < ANSWER[k].path.size(); z++)
					{
						if (ANSWER[k].path[z] == repitedly[jj].BEG && ANSWER[k].path[z + 1] == repitedly[jj].END)
		...
		con_num = condition(E[i].BEG, E[i].END, min , Optimal);
		switch (con_num)
		{
		case(0):
		{
