...
int main()
{
	...
	for (register LONG ii = 0; ii < M; ii++)
	{
		...
		if (min2 != min1) {
			...
			if (Edge[ii][2] == tmp) {	
				count = 0;
				for (register LONG jj = 0; jj < M; jj++) 
					if (Edge[ii][0] == Edge[jj][0] && Edge[ii][1] == Edge[jj][1] && Edge[ii][2] == Edge[jj][2])
						count++;
				if (count==1)	
					cout << "YES\n";
				else
		...
		min3 = (PATH[S - 1][Edge[ii][0]-1] + Edge[ii][2] + PATH[Edge[ii][1]-1][T - 1]) - min1 + 1;
		if (Edge[ii][2]-min3<1)
			cout << "NO\n";
		else
			cout << "CAN "<<min3<<"\n";
